...
- 2026-03-10: Decision — migrate test call sites to the non-deprecated projectile tick pipeline and avoid warning suppression. Root cause was stale `ProjectileSystem::update(...)` usage in `tests/projectile_tests.cpp`, `tests/entity_tests.cpp`, and `tests/boss_phase_tests.cpp`, which triggers C4996 and fails under `/WX`. Chosen migration keeps test intent while matching production sequencing each frame: `beginTick()` -> `updateMotion()` -> `buildGrid()` -> `resolveCollisions(...)` using an explicit player collision target (id/team/radius equivalent to legacy behavior). Result: deprecated call removed from all affected tests.
- 2026-03-10: Decision — preserve global strict warning policy and scope exception only to vendor stb implementation TU. Root cause was MSVC `/WX` elevating warnings emitted from `stb_image.h` and `stb_truetype.h` when compiled through `src/stb_impl.cpp`. Chosen fix: file-level CMake override `set_source_files_properties(src/stb_impl.cpp PROPERTIES COMPILE_OPTIONS "/WX-")`. Rejected alternatives: global `/WX` disable (too broad) and vendor header edits (third-party patch burden).
- 2026-03-10: Decision — keep `/W4 /WX` strictness and fix narrowing at source. In `parseLayer(...)` (`src/engine/patterns.cpp`), behavior fields intended as byte-sized counters/samples are now parsed as `unsigned`, clamped to `std::uint8_t` max, then explicitly cast. Chosen approach preserves strict compile hygiene and documents overflow handling for malformed/high JSON values.
