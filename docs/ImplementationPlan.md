...
## Phase 8 — Deterministic Math Wrappers (Conditional)
- Cross-config determinism hash comparison executed.
- Deterministic wrapper bodies implemented in `deterministic_math.cpp` to stabilize trig/sqrt code paths.

## Phase 9 — Basic Audio System
- Added SDL_mixer integration and `AudioSystem` with graceful fallback for missing audio assets.
- Wired presentation-only cues for hit, graze, boss warning, and defensive special activation.

## Phase 10 — Content Hot-Reload
- Added `ContentWatcher` timestamp polling utility.
- Generalized hot-reload coverage for patterns, entities, traits, and difficulty profiles with validate-and-swap semantics.
