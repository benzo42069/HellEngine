...
- 2026-03-10: Completed test-side projectile API migration required by `/WX` deprecation enforcement. Root cause confirmed as stale calls to `ProjectileSystem::update(...)` in `tests/projectile_tests.cpp`, `tests/entity_tests.cpp`, and `tests/boss_phase_tests.cpp`. Implemented minimal per-loop migration to the production pipeline (`beginTick`, `updateMotion`, `buildGrid`, `resolveCollisions`) while preserving existing assertions and test intent. Explicitly verified the deprecated API is no longer called in those tests.
- 2026-03-10: Completed third-party warning-policy scoping fix for stb implementation TU. Audited `engine_core` compile policy and confirmed `/W4 /WX` is target-wide under MSVC. Implemented minimal scoping update: `set_source_files_properties(src/stb_impl.cpp PROPERTIES COMPILE_OPTIONS "/WX-")` so vendor stb warnings no longer fail builds while first-party source strictness remains intact.
- 2026-03-10: Completed targeted compile-hygiene task for pattern parsing byte fields. Implemented local safe narrowing in `src/engine/patterns.cpp` for `maxBounces`, `splitCount`, `explodeShards`, and `beamSegmentSamples` using clamp-to-byte-range + explicit cast; no global warning suppression and no behavior changes outside out-of-range input normalization.
- 2026-03-09: Build-green plan update (tests/CMake): collapse Catch classification to one shared rule in `engine_link_catch_main_if_needed(...)` — no source `main()` => link `Catch2::Catch2WithMain`; source defines `main()` => standalone registration path. Remove brittle include scanning and target force-lists, and remove temporary one-off explicit links for `boss_phase_tests`/`editor_tools_tests` by routing them back through `engine_add_test(...)`. Run a full clean rebuild to regenerate link lines and Catch discovery metadata before validation.
- 2026-03-09: Closed final missing-main blocker for `pattern_graph_perf_tests`. Root cause tracked to remaining target-specific Catch main ownership drift in helper classification. Applied minimal CMake fix by adding `pattern_graph_perf_tests` to `_engine_force_catch_main_targets` so `engine_link_catch_main_if_needed(...)` guarantees `Catch2::Catch2WithMain` linkage while preserving target identity, Catch discovery, and DLL deployment semantics. Clean rebuild required to refresh generated link rules.
