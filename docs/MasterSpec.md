- 2026-03-10: Repo-wide test migration completed for deprecated `ProjectileSystem::update(...)` calls that caused MSVC C4996 deprecation warnings to fail the build under `/W4 /WX`. Migrated test files: `tests/projectile_behavior_tests.cpp`, `tests/benchmark_suite.cpp`, `tests/defensive_special_tests.cpp`, `tests/determinism_property_tests.cpp`, and `tests/determinism_cross_config_test.cpp`. Each call site now uses `beginTick()` + `updateMotion()` + `buildGrid()` + `resolveCollisions(...)` with equivalent per-test player target settings; confirmed no remaining deprecated `ProjectileSystem::update(...)` usage under `tests/`.
- 2026-03-10: Test migration for deprecated projectile API completed to restore `/W4 /WX` cleanliness. Root cause: three tests still called deprecated `ProjectileSystem::update(...)`, which emits C4996 and fails under `/WX`. Affected files: `tests/projectile_tests.cpp`, `tests/entity_tests.cpp`, and `tests/boss_phase_tests.cpp`. Migration applied in-place to explicit pipeline order per tick: `beginTick()` -> `updateMotion()` -> `buildGrid()` -> `resolveCollisions(...)` with a player collision target equivalent to legacy behavior. Confirmed deprecated `ProjectileSystem::update(...)` usage was removed from those tests.
- 2026-03-10: Build fix for MSVC `/W4 /WX` failure in third-party stb implementation unit. Root cause: `src/stb_impl.cpp` compiles vendored `stb_image.h`/`stb_truetype.h` with target-level `/WX`, so vendor warnings (for example C4244) were promoted to errors. Scoped fix: apply source-file compile option `/WX-` only to `src/stb_impl.cpp` via `set_source_files_properties(...)` in CMake. First-party engine sources remain under strict `/W4 /WX` from `engine_apply_target_defaults(...)`.
- 2026-03-10: Compile-hygiene fix for `/W4 /WX` narrowing in `src/engine/patterns.cpp` parse path. Root cause: JSON numeric fields parsed as `unsigned` were implicitly assigned into `ProjectileBehavior` byte fields (`maxBounces`, `splitCount`, `explodeShards`, `beamSegmentSamples`), triggering C4244 under strict MSVC warnings. Fix: locally clamp each parsed value to `[0, 255]` and then apply explicit `static_cast<std::uint8_t>(...)` at the assignment site. Warning policy remains unchanged.
- 2026-03-09: Systemic Catch main fix finalized in shared CMake test helper. Root cause: helper depended on Catch include-text detection plus force-lists, so Catch-based tests that did not match those heuristics could be registered as standalone executables and link without an entrypoint (`main`/LNK1120). Final rule: any test source without its own real `main()` is treated as Catch-driven and links `Catch2::Catch2WithMain`; only explicit standalone tests that define `main()` skip Catch main. Exact fix: removed include-detection/force-list path from `engine_link_catch_main_if_needed(...)`, keyed decision only on `engine_source_defines_main(...)`, and restored `boss_phase_tests`/`editor_tools_tests` to the shared `engine_add_test(...)` path so behavior is centralized. A full clean rebuild from a deleted build directory is required so regenerated linker and Catch discovery steps are applied.
- 2026-03-09: Final missing-main closure for `pattern_graph_perf_tests` completed. Root cause: target classification could still miss explicit Catch entrypoint ownership in some generated link configurations, leaving Windows link line without `Catch2::Catch2WithMain` and producing unresolved `main`/`LNK1120`. Exact CMake fix: added `pattern_graph_perf_tests` to the forced Catch-main target list inside `engine_link_catch_main_if_needed(...)`, preserving target name, Catch discovery behavior, and runtime DLL deployment helper flow. A full clean rebuild from a deleted build directory is required so regenerated linker/discovery steps are applied.
- 2026-03-09: Final missing-main closure for `editor_tools_tests` completed. Root cause: target still consumed stale/ambiguous Catch wiring and could produce a link line missing Catch entrypoint ownership, yielding unresolved `main` / `LNK1120` on Windows. Exact CMake fix: switched `editor_tools_tests` to explicit target registration (`engine_add_test_target` + explicit `target_link_libraries(... Catch2::Catch2WithMain)` + forced Catch discovery property + `engine_register_test`) while preserving target name, discovery behavior, and runtime DLL deployment helper path. A full clean rebuild from a deleted build directory is required so regenerated linker/discovery commands are applied.
...
## Build Notes
- 2026-03-09: Immediate build stabilization pass fixed remaining missing-main and empty DLL copy command failures. Root cause 1: test helper classification drift left `content_packer_tests`, `editor_tools_tests`, `boss_phase_tests`, and `entity_tests` vulnerable to unresolved `main` at link on Windows when Catch main was not guaranteed. Exact fix: `engine_link_catch_main_if_needed(...)` now force-classifies those four targets as Catch-driven while still respecting `NOT main(...)`, so they link `Catch2::Catch2WithMain` only when needed. Root cause 2: `engine_deploy_runtime_dlls(...)` always emitted `cmake -E copy_if_different` even when `$<TARGET_RUNTIME_DLLS:...>` was empty; fix adds a generator-expression guard so copy runs only when at least one runtime DLL exists. Full clean rebuild required to verify regenerated link/post-build commands.
