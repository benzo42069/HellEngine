...
namespace {
constexpr float kMaxShakeOffset = 20.0F;
...
Vec2 clampVec(const Vec2 v, const float limit) {
    const float len = vecLength(v);
    if (len <= limit || len <= 0.0001F) return v;
...
void Camera2D::update(const float dt) {
    shakeSystem_.update(std::max(dt, 0.0F));
    shakeOffset_ = clampVec(shakeSystem_.offset(), kMaxShakeOffset);
}
...
void CameraShakeSystem::update(const float dt) {
    currentOffset_ = {0.0F, 0.0F};
    ...
    for (ActiveShake& shake : shakes_) {
        ...
        currentOffset_.x += local.x;
        currentOffset_.y += local.y;
    }
    currentOffset_ = clampVec(currentOffset_, kMaxShakeOffset);
}

Vec2 CameraShakeSystem::offset() const { return currentOffset_; }
