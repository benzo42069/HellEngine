...
void Camera2D::setShake(const float amplitude, const float seconds) {
    ...
    shakeTimeRemaining_ = std::max(seconds, 0.0F);
}

void Camera2D::update(const float dt) {
    if (shakeTimeRemaining_ <= 0.0F || shakeAmplitude_ <= 0.0F) {
        shakeOffset_ = {0.0F, 0.0F};
        return;
    }

    shakeTimeRemaining_ = std::max(0.0F, shakeTimeRemaining_ - dt);
    const float phase = shakeTimeRemaining_ * 45.0F;
    shakeOffset_.x = std::sin(phase) * shakeAmplitude_;
    shakeOffset_.y = std::cos(phase * 0.87F) * shakeAmplitude_;
}
