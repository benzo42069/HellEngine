...
void Runtime::simTick(const double dt) {
    session_.processPendingContentReloads();
...
int Runtime::run() {
    ...
    session_.initializeContentWatcher();

    (void)audioSystem_.initialize();
    hitSound_ = audioSystem_.loadSound("assets/audio/hit.wav");
    grazeSound_ = audioSystem_.loadSound("assets/audio/graze.wav");
    warningSound_ = audioSystem_.loadSound("assets/audio/warning.wav");
    specialSound_ = audioSystem_.loadSound("assets/audio/special.wav");
    if (const ZoneDefinition* zone = session_.runStructure_.currentZone()) {
        lastZoneType_ = zone->type;
    }
    ...
    while (running_) {
        ...
        if (session_.projectiles_.stats().collisionsThisTick > 0U) audioSystem_.playSound(hitSound_, 0.6F);
        if (session_.grazePointsThisTick_ > 0U) audioSystem_.playSound(grazeSound_, 0.5F);
        if (session_.defensiveSpecialActivatedThisTick_) audioSystem_.playSound(specialSound_, 0.7F);
        if (const ZoneDefinition* zone = session_.runStructure_.currentZone()) {
            if (zone->type == ZoneType::Boss && lastZoneType_ != ZoneType::Boss) {
                audioSystem_.playSound(warningSound_, 0.8F);
            }
            lastZoneType_ = zone->type;
        }
