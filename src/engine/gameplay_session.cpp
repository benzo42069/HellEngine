...
void GameplaySession::processPendingContentReloads() {
    hotReloadErrorMessage_.clear();
    const std::vector<ContentChange> changes = contentWatcher_.pollChanges();
    for (const ContentChange& change : changes) {
        switch (change.type) {
            case ContentType::Patterns: {
                PatternBank loaded;
                if (!loaded.loadFromFile(change.path)) {
                    hotReloadErrorMessage_ = "Pattern hot-reload failed: " + change.path;
                    logError(hotReloadErrorMessage_);
                    break;
                }
                patternBank_ = std::move(loaded);
                patternPlayer_.setBank(&patternBank_);
                logInfo("Pattern hot-reload applied: " + change.path);
                break;
            }
            case ContentType::Entities: {
                EntityDatabase loaded;
                if (!loaded.loadFromFile(change.path)) {
                    hotReloadErrorMessage_ = "Entity hot-reload failed: " + change.path;
                    logError(hotReloadErrorMessage_);
                    break;
                }
                entityDatabase_ = std::move(loaded);
                entitySystem_.setTemplates(&entityDatabase_.templates());
                logInfo("Entity hot-reload applied: " + change.path);
                break;
            }
            case ContentType::Traits: {
                TraitSystem loaded = traitSystem_;
                if (!loaded.loadFromFile(change.path)) {
                    hotReloadErrorMessage_ = "Trait hot-reload failed: " + change.path;
                    logError(hotReloadErrorMessage_);
                    break;
                }
                traitSystem_ = std::move(loaded);
                logInfo("Trait hot-reload applied: " + change.path);
                break;
            }
            case ContentType::Difficulty: {
                DifficultyModel loaded = difficultyModel_;
                bool success = false;
                try {
                    success = loaded.loadProfilesFromFile(change.path);
                } catch (...) {
                    success = false;
                }
                if (!success) {
                    hotReloadErrorMessage_ = "Difficulty hot-reload failed: " + change.path;
                    logError(hotReloadErrorMessage_);
                    break;
                }
                difficultyModel_ = std::move(loaded);
                logInfo("Difficulty hot-reload applied: " + change.path);
                break;
            }
        }
    }
...
void GameplaySession::updateGameplay(const double dt, const std::uint32_t inputMask) {
    ...
    *scratch = tickIndex_;
    ...
    defensiveSpecialActivatedThisTick_ = false;
    if ((inputMask & InputDefensiveSpecial) != 0U) {
        defensiveSpecialActivatedThisTick_ = defensiveSpecial_.tryActivate();
    }
    ...
    grazePointsThisTick_ = projectiles_.collectGrazePoints(playerPos_, playerRadius_, defensiveSpecial_.config().grazeBandInnerPadding, defensiveSpecial_.config().grazeBandOuterPadding, tickIndex_, defensiveSpecial_.config().grazeCooldownTicks);
    if (grazePointsThisTick_ > 0) defensiveSpecial_.addGrazePoints(grazePointsThisTick_);
